...
class JobWorkerTests(unittest.TestCase):
    def test_http_video_job_reaches_succeeded(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            fake_runner = Path(__file__).with_name("fake_video_generate.py")
            old_runner = os.environ.get("DOLA_VIDEO_GENERATE_RUNNER")
            old_probe = os.environ.get("DOLA_SKIP_FFPROBE")
            os.environ["DOLA_VIDEO_GENERATE_RUNNER"] = str(fake_runner)
            ...
            try:
                ...
                if old_runner is None:
                    os.environ.pop("DOLA_VIDEO_GENERATE_RUNNER", None)
                else:
                    os.environ["DOLA_VIDEO_GENERATE_RUNNER"] = old_runner
