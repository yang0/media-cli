...
class Worker:
    ...
    def _run_job(self, job: dict) -> None:
        ...
        # Only free_protocol HTTP submit (dola-free style). The old inject_shell
        # auto-UI path is retired: it triggers captcha/logout and is unused.
        # Image-to-video still opens a short-lived WebView ONLY to upload refs
        # (no chat submit), then POSTs /chat/completion externally.
        #
        # DOLA_VIDEO_GENERATE_RUNNER may point at a test double script; production
        # always uses `python -m free_protocol.cli`.
        runner = (os.environ.get("DOLA_VIDEO_GENERATE_RUNNER") or "").strip()
        if runner:
            command = [sys.executable, runner]
        else:
            command = [sys.executable, "-m", "free_protocol.cli"]
        command.extend(
            [
                "video",
                "generate",
        ...
        mode_note = f"mode=http-external-submit refs={len(refs)}"
        env = os.environ.copy()
        env.update(
            {
                "DOLA_JOB_MANAGED": "1",
        ...
        if not self.store.set_submitting(job_id):
            ...
            return
        _append_log(log_file, f"[{now_iso()}] {mode_note}")
