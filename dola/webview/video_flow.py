...
def run_video_generation(
    ...
    *,
    ...
    # Dismiss blocking modals (e.g. "我知道了") that hide the video composer.
    dismiss_js = r"""
(() => {
...
"""
    try:
        _log(log, f"dismiss overlays: {js_eval(window, dismiss_js)}")
    except Exception as exc:
        _log(log, f"dismiss overlays failed: {exc}")
    time.sleep(0.5)
    ...
    time.sleep(2.0)
    try:
        _log(log, f"dismiss after video: {js_eval(window, dismiss_js)}")
    except Exception:
        pass
    # Wait until the video skill mounts file inputs / ratio controls.
    for warm in range(15):
        n = js_eval(window, "document.querySelectorAll('input[type=file]').length")
        labels = js_eval(
            window,
            "Array.from(document.querySelectorAll('button,[role=button]')).map(e=>(e.innerText||'').trim()).filter(Boolean).slice(0,20)",
        )
        _log(log, f"video UI warm[{warm}]: fileInputs={n} labels={labels}")
        if int(n or 0) > 0:
            break
        time.sleep(0.5)

    # Align duration + ratio with native controls (defaults are often 10s / 1:1).
    duration_label = f"{int(duration)}s"
    duration_ui_ok = False
    for dur_attempt in range(1, 4):
        duration_ui_ok = select_video_ui_option(
            window,
            control_keys=["video-duration", "duration", "video-length"],
            value=duration_label,
            log=log,
        )
        if duration_ui_ok:
            break
        _log(log, f"duration UI retry={dur_attempt}/3 requested={duration_label}")
        time.sleep(0.6)
    _log(log, f"duration UI selected={duration_ui_ok} requested={duration_label}")

    ratio_ui_ok = False
    ratio_keys = ["video-ratio", "video-aspect-ratio", "aspect-ratio", "ratio"]
    for ratio_attempt in range(1, 5):
        ...
        if ratio_ui_ok:
            break
        # Also try clicking the 比例 trigger by visible text first.
        if ratio_attempt == 2:
            js_eval(
                window,
                r"""
(() => {
  ...
  const btn = Array.from(document.querySelectorAll('button, [role=button], div, span'))
    .filter(visible)
    .find(el => text(el) === '比例' || text(el).startsWith('比例'));
  if (!btn) return { ok: false, reason: 'ratio-label-missing' };
  btn.click();
  return { ok: true, text: text(btn) };
})()
""",
            )
            time.sleep(0.8)
        _log(log, f"aspect ratio UI retry={ratio_attempt}/4 requested={ratio}")
        time.sleep(0.8)
    _log(log, f"aspect ratio UI selected={ratio_ui_ok} requested={ratio}")
    if ratio and not ratio_ui_ok:
        _log(
            log,
            f"WARN could not confirm aspect ratio {ratio} in Dola UI; "
            f"continuing with completion patch ratio fallback",
        )
    ...
    if refs:
        try:
            ...
            raise
        # Capture ImageX commit so we never submit before the ref is uploaded.
        upload_capture_js = r"""
(() => {
  if (window.__dolaRefUploadCap) return { ok: true, reused: true, commits: (window.__dolaRefCommits||[]).length };
  window.__dolaRefCommits = [];
  window.__dolaRefUploadCap = true;
  const mark = (url, status, response) => {
    try {
      if (/CommitImageUpload/i.test(String(url||'')) && Number(status) === 200) {
        window.__dolaRefCommits.push({ t: Date.now(), url: String(url||'').slice(0,200), response: String(response||'').slice(0,4000) });
      }
    } catch (e) {}
  };
  ...
  XMLHttpRequest.prototype.send = function(body) {
    const url = this.__u || '';
    this.addEventListener('loadend', () => mark(url, this.status, this.responseText));
...
"""
        try:
            _log(log, f"ref-upload-capture: {js_eval(window, upload_capture_js)}")
        except Exception as exc:
            _log(log, f"ref-upload-capture install failed: {exc}")
        ...
        need_commits = max(1, len(files))
        upload_deadline = time.time() + 90
        commits = 0
        while time.time() < upload_deadline:
            try:
                commits = int(
                    js_eval(window, "(window.__dolaRefCommits && window.__dolaRefCommits.length) || 0") or 0
                )
            except Exception:
                commits = 0
            try:
                ...
                st = {"err": str(exc)}
            _log(log, f"ref-upload wait commits={commits}/{need_commits} send={st}")
            if commits >= need_commits and isinstance(st, dict) and st.get("sendEnabled"):
                _log(log, f"reference upload committed ({commits}) and send enabled")
                break
            time.sleep(0.8)
        else:
            raise RuntimeError(
                f"reference image upload did not commit in time "
                f"(CommitImageUpload {commits}/{need_commits}); not submitting"
            )
        # Brief settle so SPA attaches the uploaded entity to the composer.
        time.sleep(1.5)
    else:
    ...
    # Install lightweight completion-body capture before submit (debug i2v shapes).
    capture_completion_js = r"""
(() => {
  if (window.__dolaCompletionCapture) return { ok: true, reused: true };
  window.__dolaCompletionBodies = [];
  window.__dolaCompletionResponses = [];
  window.__dolaCompletionCapture = true;
  const XO = XMLHttpRequest.prototype.open;
  const XS = XMLHttpRequest.prototype.send;
  ...
  XMLHttpRequest.prototype.send = function(body) {
    ...
    if (/\/chat\/completion/i.test(url)) {
      try {
        window.__dolaCompletionBodies.push({
          t: Date.now(),
          url: url.slice(0, 300),
          body: typeof body === 'string' ? body.slice(0, 20000) : String(body).slice(0, 500),
        });
      } catch (e) {}
      this.addEventListener('loadend', () => {
        try {
          window.__dolaCompletionResponses.push({
            t: Date.now(),
            status: this.status,
            response: String(this.responseText || '').slice(0, 8000),
          });
        } catch (e) {}
      });
    }
  ...
  window.fetch = async function(input, init) {
    const url = typeof input === 'string' ? input : (input && input.url) || '';
    if (/\/chat\/completion/i.test(String(url))) {
      try {
    ...
    if (/\/chat\/completion/i.test(String(url))) {
      p.then(async (resp) => {
        let text = '';
        try { text = (await resp.clone().text()).slice(0, 8000); } catch (e) {}
        window.__dolaCompletionResponses.push({ t: Date.now(), status: resp.status, response: text });
      }).catch(() => {});
    }
    return p;
  };
...
"""
    try:
        _log(log, f"completion-capture: {js_eval(window, capture_completion_js)}")
    except Exception as exc:
        _log(log, f"completion-capture install failed: {exc}")
    ...
    for attempt in range(1, 10):
        ...
        if isinstance(st2, dict) and int(st2.get("promptLen") or 0) < max(8, len(prompt) // 3):
            ...
            try:
                bodies = js_eval(window, "window.__dolaCompletionBodies || []") or []
                resps = js_eval(window, "window.__dolaCompletionResponses || []") or []
                _log(log, f"completion bodies captured={len(bodies)} responses={len(resps)}")
                if bodies:
                    last_body = bodies[-1].get("body") if isinstance(bodies[-1], dict) else ""
                    _log(log, f"completion body sample: {str(last_body)[:1200]}")
                    try:
                        out_path = Path(out_dir) / "last_completion_body.json"
                        out_path.write_text(str(last_body), encoding="utf-8")
                        _log(log, f"wrote {out_path}")
                    except Exception as exc:
                        _log(log, f"write completion body failed: {exc}")
                if resps:
                    last_resp = resps[-1]
                    _log(log, f"completion response sample: {str(last_resp)[:800]}")
            except Exception as exc:
                _log(log, f"read completion capture failed: {exc}")
            submitted = True
            try:
