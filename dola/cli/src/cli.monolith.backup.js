...
const VIDEO_MODEL_SEEDANCE_V2 = "seedance_v2.0";
const VIDEO_ABILITY_TYPE = 17;
const DOLA_MEDIA_AID = "489823";
...
Options:
  ...
  --duration <seconds>     Video duration: 5, 10, or 15 (default: 5). 15s is not in the official UI;
                           the CLI patches POST /chat/completion like the Dola reseller tools.
  --model <name>           Video model override for the completion payload (default: seedance_v2.0 when duration is 15).
...
function parseArgs(argv) {
  ...
  if (args.videoGen) {
    ...
    if (args.model !== undefined) {
      ...
      // 15s is only reliably accepted with Seedance v2 (same as the reference desktop tool).
...
function normalizeCookieDomain(domain, includeSubdomains = false) {
...
async function inspectCookieFile(file) {
...
function loadPoolDayState(savedState, day = accountDayKey()) {
...
function markAccountBlocked(poolDayState, accountId, reason = "restricted") {
  if (!accountId) return;
...
function bumpAccountUsage(poolDayState, accountId, amount = 1) {
  if (!accountId) return;
  const id = String(accountId);
  poolDayState.usage[id] = (Number(poolDayState.usage[id]) || 0) + amount;
}

function chooseAccount(accounts, poolDayState, preferredId = "", options = {}) {
...
function accountStateFields(accountPoolFile, activeAccount, poolDayState) {
...
async function listAccountPoolStatus(accountPoolFile, savedState = null) {
...
async function detectDolaLoginState(client) {
...
function rewriteVideoNoWatermarkParams(url) {
...
function videoModeReadyExpression() {
  ...
  return `(() => {
    ...
    // Composer area often shows Seedance / duration / ratio after video mode is on.
    const chips = Array.from(document.querySelectorAll("button, [role='button'], [role='tab'], a, div, span")).filter(visible);
    ...
    // Strict chip active state only (avoid matching class names that merely contain "primary").
...
async function startFreshChat(client) {
  ...
  const clicked = await evaluate(client, `(() => {
    ...
    const el = nodes.find(node => {
      const t = plain(node);
      return labels.some(label => t === label || t.startsWith(label)) && t.length <= 20;
    });
...
async function ensureVideoGenerationMode(client) {
  ...
  if (!controlsReady?.ready) {
    throw new Error(
      `Video generation form did not appear after selecting the video mode (${controlsReady?.reason || "unknown"}).`
      + " Make sure the composer shows video controls (for example Seedance / 5s / 10s)."
    );
  }
 ...
 */
function resolveVideoGenConfig(options = {}) {
  ...
  return {
    duration,
    model,
    aspectRatio: options.aspectRatio ? String(options.aspectRatio).trim() : "",
    /** Always patch completion body when generating video so duration/model stick. */
    patchCompletion: true,
  };
}

function buildVideoCompletionPatchScript(config) {
  ...
  return `(() => {
    ...
    if (window[MARK] && window[MARK].duration === TARGET_DURATION && window[MARK].model === TARGET_MODEL) {
    ...
    function patchDuration(obj, depth) {
      ...
      if (obj.chat_ability && Number(obj.chat_ability.ability_type) === ABILITY_TYPE) {
        ...
        if (typeof ability.ability_param === "string") {
          ...
          if (TARGET_MODEL) ability.ability_param.model = TARGET_MODEL;
    ...
    patchFetch();
    patchXhr();
    window[MARK] = { duration: TARGET_DURATION, model: TARGET_MODEL, abilityType: ABILITY_TYPE, at: Date.now() };
    return { ok: true, reused: false, ...window[MARK] };
  })()`;
}

async function installVideoRequestPatch(client, options = {}) {
...
async function selectVideoOptions(client, options = {}) {
  ...
  if (aspectRatio) {
    if (await choose("video-ratio", aspectRatio)) result.push(aspectRatio);
    else {
      // UI control keys/layout change often; do not hard-fail once video mode is on.
      console.log(`[dola-cli] video aspect-ratio UI option not found (${aspectRatio}); continuing without UI selection`);
      result.push(`ratio-ui:missing`);
    }
  }
 ...
 */
async function installVideoResolveHelpers(client) {
...
function parseRemainingQuota(text) {
  const value = String(text || "");
  const patterns = [
...
async function main() {
  ...
  if (args.listAccounts) {
    if (!accountPoolFile) throw new Error("--list-accounts requires --account-pool.");
    const status = await listAccountPoolStatus(accountPoolFile, savedState);
    console.log(JSON.stringify(status, null, 2));
    return;
  }
  if (args.accountId && !accountPoolFile) throw new Error("--account-id requires --account-pool.");
  ...
  const switchRestrictedAccount = async (error, lineNumber) => {
    ...
    sessionUrl = normalizeSession(args.videoGen ? DOLA_CHAT_HOME : (args.imageGen ? DOLA_IMAGE_HOME : DOLA_CHAT_HOME));
