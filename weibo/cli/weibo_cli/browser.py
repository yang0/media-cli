"""Small Chrome DevTools Protocol transport shared by search auth and capture."""
...
def http_json(url: str, *, method: str = "GET", data: bytes | None = None, timeout: float = 5) -> Any:
    ...
    try:
        ...
        # Never include a URL query in an auth error: it could contain a user
        # supplied token in a future endpoint.
...
def discover_browser_executable() -> str | None:
    """Find a locally installed Chrome/Edge executable on Windows."""

    candidates: list[Path] = []
    for env_name in ("WEIBO_CHROME_EXECUTABLE", "CHROME_PATH", "PROGRAMFILES", "PROGRAMFILES(X86)", "LOCALAPPDATA"):
        value = os.environ.get(env_name)
        if not value:
            continue
        root = Path(value)
        if env_name in {"WEIBO_CHROME_EXECUTABLE", "CHROME_PATH"}:
...
class CdpSession:
    """Async websocket CDP command session."""
...
async def wait_for_page(session: CdpSession, *, seconds: float = 5, interval: float = 0.25) -> None:
...
async def _read_cookies_async(port: int) -> str:
    tabs = list_tabs(port)
    target = next((tab for tab in tabs if "weibo" in str(tab.get("url", ""))), None)
    ...
    scoped = [
        ...
        and item.get("name")
        and "weibo" in str(item.get("domain", "")).lower()
    ]
    # Some Chrome builds omit ``domain`` in a manually created cookie.  In
    # that case retain a value only when the selected tab itself is a Weibo
    # page, which is already enforced above.
    if not scoped and "weibo" in str(target.get("url", "")).lower():
        scoped = [item for item in cookies if isinstance(item, dict) and item.get("name")]
    parts = [f"{item.get('name')}={item.get('value', '')}" for item in scoped]
    return "; ".join(parts)
...
def cdp_cookie_header(port: int = 9221) -> str:
    try:
        ...
        # ``asyncio.run`` may be called from a host loop (e.g. notebooks).  A
        # separate thread would be overkill for this CLI; surface a redacted
        # and deterministic failure instead of leaking session data.
        raise CdpError("当前运行环境已有活动事件循环，无法读取 Chrome Cookie") from exc


class TemporaryChrome:
    """Optional owner-managed Chrome process for cookie-backed capture.

    It is intentionally a small boundary object.  The CLI uses it only when
    explicitly asked to fall back from CDP; it never closes a user's Chrome.
    """
    ...
    def __init__(self, *, executable: str | None = None, port: int | None = None, profile_dir: str | Path | None = None):
        ...
        self.profile_dir = Path(profile_dir) if profile_dir else Path(tempfile.mkdtemp(prefix="weibo-cli-chrome-"))
    ...
    def start(self) -> int:
        ...
        self.process = subprocess.Popen(
            [self.executable, f"--remote-debugging-port={self.port}", f"--user-data-dir={self.profile_dir}", "about:blank"],
