"""9:16-aware screenshot slicing with content-boundary preference."""
...
def plan_slices(width: int, height: int, boundaries: Iterable[Any] = (), *, overlap: int = 64) -> list[CropSlice]:
    """Return crop coordinates; every crop is at most width:height = 9:16."""

    if width <= 0 or height <= 0:
        raise ValueError("图片尺寸必须为正数")
    if overlap < 0:
        raise ValueError("重叠像素不能为负数")
    max_height = max_height_for_ratio(width)
    ...
    while current < height:
        ...
        candidates = [point for point in points if current + max_height // 2 <= point <= limit and point > current]
        if limit == height:
            end = height
            mode = "final"
            next_current = height
...
def split_image_bytes(image_bytes: bytes, boundaries: Iterable[Any] = (), *, overlap: int = 64, output_dir: str | Path | None = None, weibo_id: str = "weibo") -> list[dict[str, Any]]:
    """Split PNG/JPEG bytes and optionally write numbered PNG pieces."""
    ...
    for piece in pieces:
        ...
        filename = f"weibo-{weibo_id}-{piece.index:02d}-of-{piece.total:02d}.png"
        path = None
        if output:
